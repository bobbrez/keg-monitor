var impAgentURL = "https://agent.electricimp.com/AGENT-TOKEN";

var gauges = {};

var pubnub = PUBNUB({
  subscribe_key: 'sub-c-24c7195a-cd55-11e5-a9aa-02ee2ddab7fe',
  publish_key: 'pub-c-65c69104-35f5-42f3-a2fd-5b3eac654653'
});
...
pubnub.subscribe({
   ...
   error: function (error) { console.warn(JSON.stringify(error)); }
});

$.getJSON(impAgentURL, function(data) { 
  console.log(data);
  updateGauges(data);
});

function initialize() {
  createGauge("keg0", "", 100);
  createGauge("keg1", "", 100);
  createGauge("keg2", "", 100);
}
...
function updateGauges(data) {
  ...
  for (var key in data) {
    var value = data[key].currentVolume / data[key].totalVolume * 100;
    if(value < 0) { value = 0; }

    if (value < 11) {
      gauges[key].drawBand(0,value, "#DC3912");
    } else if(10 < value && value < 26) {
      gauges[key].drawBand(0, value, "#FF9900");
    } else {
      gauges[key].drawBand(0, value, "#109618");
    }
